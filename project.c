...
struct struct_ticket
{
    ...
    char date_ticket[20];
    char replay[200];
    char status[3];//0 :=not replay yet 1:= get replay 2:= get repley ande readed by user
...
int list_unread_ticket_by_user_name(char user[])
{
    ...
    int i=1;
        ...
        if (i==1)
        {
            printf("\t\t\t   not unread ticket exist!\n");
            printf("Press Enter to contniue\n");
            press_enter_to_continue();
            return -1;
        }
...
int answer_panle_unread_ticket_print(int num,char user[])
{
    char temp[25],char_line='|';
    int i=1;
        printf("\t\t\t      Ticket massage\n--------------------------------------------------------------------------\n");
        ...
        } while (temp_struct_ticket!=NULL);
        {
            ...
            if (temp_struct_ticket->status[0]!='0')
            {
                ...
                printf("Stutus ticket: Waiting for OP answer\n");
        }
...
void add_linked_list_academic_to_notpadd()
{
    ...
    temp_struct_academic=start_struct_academic;
    if (temp_struct_academic->name[0]=='0')
    {
        fclose(Academic);
        free_academic();
        return ;
...
void add_linked_list_departemant_to_notpadd()
{
    ...
    temp_struct_departemant=start_struct_departemant;
    if (temp_struct_departemant->name[0]=='0')
    {
        fclose(Departemant);
        free_departemant();
        return ;
...
void add_linked_list_student_to_notpadd()
{
    ...
    temp_struct_student=start_struct_student;
    if (temp_struct_student->name[0]=='0')
    {
        fclose(Student);
        free_student();
        return ;
...
void settings_departemant()
{
    ...
    printf("if you want Exit from proses just perss enter\n");
        ...
        {
            password_to_star(str_temp);
            flag=check_strong_password(str_temp);
            if (strlen(str_temp)==0)
                break;
        ...
        do
        {
            ...
            {
                if (strcmp(str_temp,temp_struct_departemant->phone_num)==0 && strlen(str_temp)==strlen(temp_struct_departemant->phone_num))
                {
...
void settings_academic()
{
    ...
    printf("if you want Exit from proses just perss enter\n");
        ...
        do
        {
            ...
            {
                if (strcmp(str_temp,temp_struct_academic->phone_num)==0 && strlen(str_temp)==strlen(temp_struct_academic->phone_num))
                {
                    printf("new Phone should not be same as old Phone! Try another Phone: ");
                    continue;
                }
...
void list_course_special_print(int status)// status1 := D and 0:= E
{
    ...
    int i=1;
        ...
        do
        {
            ...
            else if (status==0 && temp_struct_course->status[0]=='E')
            {
                printf("Course's %d\n",i++);
                printf("-----------------------------------\n");
                strcpy(temp,"Name Course");
...
void calcurat_avrage_score_student()
{
    ...
    do{
        ...
        if (counter!=0)
            ...
            strcpy(temp_struct_student->avg_score,"0");
...
void student_passed_search_by_code_cours_print()
{
    char code[11];
    printf("if you can cancel prosses, just press Enter\n");
    printf("enter Code Course: ");
    ...
    temp_struct_score=start_struct_score;
    float min_score=10.00;
    printf("   list of stuednt passed\n");
    printf("__________________________________________\n");
    do
    {
        if (strcmp(temp_struct_score->code_course,code)==0 && strlen(temp_struct_score->code_course)==strlen(code) && atof(temp_struct_score->score)>=min_score)
        {
            counter++;
            search_user_name_student(temp_struct_score->ID_uni);
            strcpy(temp,"Name Student: ");
...
void student_dont_passed_search_by_code_cours_print()
{
    char code[11];
    printf("if you can cancel prosses, just press Enter\n");
    printf("enter Code Course: ");
...
void show_list_student_mashroot()
{
    ...
    int i=1;
        ...
        float min_score=12.00;
        temp_struct_student=start_struct_student;
        do
        {
            if (temp_struct_student->name[0]=='0')//file is NULL
                break;
            if (atof(temp_struct_student->avg_score)<min_score)
            {
                printf("Student's %d\n",i++);
                printf("-----------------------------------\n");
                strcpy(temp,"Name: ");
...
void show_list_student_mashroot_and_get_score_in_one_cours()
{
    char code[11];
    printf("if you can cancel prosses, just press Enter\n");
    printf("enter Code Course: ");
    ...
    int i=1;
        ...
        do
        {
            ...
            if (atof(temp_struct_student->avg_score)<min_score && search_user_name_student_sync_with_code_sourse(temp_struct_student->ID_code,code)==0)
            {
                printf("Student's %d\n",i++);
                printf("-----------------------------------\n");
                strcpy(temp,"Name: ");
...
void list_student_search_major_print()
    {
    char major[20];
    printf("if you want cancel proses just press Enter\n");
    ...
    char temp[25];
    do{
        if (strcmp(major,temp_struct_student->major)==0 && strlen(major)==strlen(temp_struct_student->major))
        {
            printf("Student's %d",i++);
            printf("\n-----------------------------------\n");
            strcpy(temp,"Name");
...
void list_student_search_city_print()
{
    ...
    do
    {
        ...
        if (strlen(city)==0)
            return ;
        else if (check_str_full_alpha_whit_space(city))
            printf("Invalid from input!! try again: ");
        else
            break;
    } while (1);
    ...
    char temp[25];
    do{
        if (strcmp(city,temp_struct_student->location_born)==0 && strlen(city)==strlen(temp_struct_student->location_born))
        {
            printf("Student's %d",i++);
            printf("\n-----------------------------------\n");
            strcpy(temp,"Name");
    ...
    if (i==1)
    {
        printf("location born None of student is %s!\n",city);
        printf("--------------------------------------\n");
    }
    printf("Press Enter to back list\n");
    free_student();
